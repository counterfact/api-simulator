...
Counterfact generates typed response builders and can return example data from the spec, but has no concept of streaming or server-sent events. The `text/event-stream` content type is not specially handled in `packages/counterfact/src/typescript-generator/response-type-coder.ts`, `packages/counterfact/src/typescript-generator/schema-type-coder.ts`, or the Koa layer.

## Proposed changes

- Recognise `itemSchema` in `packages/counterfact/src/typescript-generator/schema-type-coder.ts` and `packages/counterfact/src/typescript-generator/response-type-coder.ts` and generate `Array<T>` (or `AsyncIterable<T>`) types for it
- In the server layer (`packages/counterfact/src/server/koa-middleware.ts`), when a request accepts `text/event-stream`, send an SSE-formatted response using Node.js streams instead of a one-shot JSON body
